...
main(int32_t argc, char *argv[])
{
    ...
    /* sanity check */
    DIE(argc != 3, "Usage: ./a.out /proc/*/ns/net");

    /* Create csv file for plotting */
    FILE *csv = fopen(argv[2], "w");
    DIE(csv == NULL, "failed to open output file (%s)", strerror(errno));

    fprintf(csv, "second,rtt_us,rttvar_us,cwnd,snd_wnd,bytes_sent,delivery_rate_bps\n");
    ...
    /* Destination data for h3 */
    inet_pton(AF_INET, "10.0.1.101", &conn_req.id.idiag_src);   /* src ip addr filter  */
    inet_pton(AF_INET, "10.0.2.101", &conn_req.id.idiag_dst);   /* dst ip addr filter  */
    conn_req.id.idiag_dport  = htons(5201);                     /* dst port filter     */
    ...
    for (int t = 0; t < 60; t++) {
        ...
        while(NLMSG_OK(nlh_it, ans)) {
            ...
            if (nlh_it->nlmsg_type == NLMSG_DONE || nlh_it->nlmsg_type == NLMSG_ERROR)
                break;
            ...
            int rta_len = nlh_it->nlmsg_len - NLMSG_LENGTH(sizeof(*diag_msg));
            ...
            for (; RTA_OK(attr, rta_len); attr = RTA_NEXT(attr, rta_len)) {
                if (attr->rta_type == INET_DIAG_INFO) {
                    ...
                    // printf("RTT              : %u µs\n", info->tcpi_rtt);
                    // printf("Min RTT          : %u µs\n", info->tcpi_min_rtt);
                    // printf("RTT variance     : %u µs\n", info->tcpi_rttvar);
                    // printf("Cong. Window     : %u segments\n", info->tcpi_snd_cwnd);
                    // printf("Send Window      : %u\n", info->tcpi_snd_wnd);
                    // printf("Bytes Acked      : %llu\n", info->tcpi_bytes_acked);
                    // printf("Bytes Sent       : %llu\n", info->tcpi_bytes_sent);
                    // printf("Delivery Rate    : %llu Bps\n", info->tcpi_delivery_rate);
                    // printf("---------------------\n");

                    fprintf(csv, "%d,%u,%u,%u,%u,%llu,%llu\n",
                        t,
                        info->tcpi_rtt,
                        info->tcpi_rttvar,
                        info->tcpi_snd_cwnd,
